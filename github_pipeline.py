...
def load_dlthub_dlt_all_data() -> None:
    """Loads all issues, pull requests, stargazers and comments for dlthub dlt repo"""
    pipeline = dlt.pipeline(
        "github_reactions",
        destination='bigquery',
...
def load_dlthub_dlt_stargazers() -> None:
    ...
    pipeline = dlt.pipeline(
        "github_reactions",
        destination='bigquery',
        dataset_name="check_stars",
        full_refresh=True,
    )
    data = github_reactions("dlt-hub", "dlt").with_resources("stargazers")
    print(pipeline.run(data))
