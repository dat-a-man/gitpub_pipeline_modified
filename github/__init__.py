...
from .queries import ISSUES_QUERY, PR_QUERY, RATE_LIMIT, COMMENT_REACTIONS_QUERY, STARGAZERS_QUERY
...
def github_reactions(
    ...
    return (
        ...
        dlt.resource(  # Add a new resource for stargazers
            _get_stargazers_data(
                owner,
                name,
                access_token,
                items_per_page,
...
def _get_reactions_data(
    ...
    max_items: int,
    max_item_age_seconds: float = None,
    since: datetime = None  # new parameter, default to None
) -> Iterator[Iterator[StrAny]]:
    if since is None:
        since = datetime.utcnow().isoformat() + "Z"  # set to current time if not provided
    else:
        since = since.isoformat() + "Z"  # convert to ISO 8601 format
    ...
    variables = {
        ...
        "first_comments": 100,
        "node_type": node_type,
        "since": since  # using the since variable here
    }
...
def _get_reactions_data_pr(
    ...
    for page_items in _get_graphql_pages(
        access_token, PR_QUERY % node_type, variables, node_type, max_items
    ):
        # use reactionGroups to query for reactions to comments that have any reactions. reduces cost by 10-50x
        reacted_comment_ids = {}
        for item in page_items:
...
def _get_stargazers_data(
    ...
    variables = {
        "owner": owner,
        "name": name,
        "stargazers_per_page": items_per_page,
    }
    for page_items in _get_graphql_pages(
...
def _extract_nested_nodes(item: DictStrAny) -> DictStrAny:
    ...
    item["comments_totalCount"] = comments.get("totalCount", 0)
    ...
    item["comments"] = comments.get("nodes", [])
    return item
...
def _get_graphql_pages(
    ...
    items_count = 0
    end_cursor = None  # Initialize end_cursor to None
    while True:
        if end_cursor:
            variables["page_after"] = end_cursor  # Set the end_cursor in variables
        ...
        if data_items:
            ...
            return
        # Extract the endCursor from the response data for pagination
        page_info = _extract_top_connection(data, node_type).get("pageInfo", {})
        end_cursor = page_info.get("endCursor")
        if not end_cursor or (max_items and items_count >= max_items):
